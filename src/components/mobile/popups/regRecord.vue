<template>
    <div class="record">
        ...
        <div class="record__item">
            ...
            <div>{{ props.item.registrationType }}</div>
        </div>
        <div class="record__item">
        ...
        <div class="record__item">
            <div class="record__item__title">就诊时间</div>
            <div>{{ props.item.registrationAt }}</div>
        </div>
        <div class="btn" @click.prevent="confirmDeletePop">删除</div>
    </div>
    <van-popup v-model:show="showPop" round >
        <div class="delete__confirm">
            <div class="delete__confirm__title">确定要删除此挂号吗？</div>
...
import { deleteRegistrationOrder } from '@/api/index';
...
const deleteOrder = () => {
    ...
    deleteRegistrationOrder(`/order-registration/${props.item.order_id}`).then(
        res => {
